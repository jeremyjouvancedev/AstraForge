...
- `ghcr.io/<namespace>/astraforge-docs`
...
services:
  ...
  docs:
    image: ghcr.io/jeremyjouvancedev/astraforge-docs:latest
    ports:
      - "3000:80"
    networks:
      - astraforge
...
- Swap Postgres/Redis for external services if you already host them; update the URLs accordingly.
- Set DNS for `astraforge.example.com` (or your preferred hostname) to the Portainer host so nginx can route traffic. Add TLS by mounting certs/keys and updating the nginx config with `listen 443 ssl` plus `ssl_certificate` / `ssl_certificate_key`.
