...
- OpenAI-compatible credentials or an Ollama daemon for the LLM proxy
...
- Copy `.env.example` to `.env` and set `SECRET_KEY`, `DATABASE_URL`, `REDIS_URL`, `LLM_PROVIDER`, `OLLAMA_BASE_URL` + `OLLAMA_MODEL` (or `OPENAI_API_KEY` + `LLM_MODEL`), and `PROVISIONER` (`docker` or `k8s`).
...
- For Docker sandboxes, keep `SANDBOX_DOCKER_READ_ONLY=1` and `SANDBOX_DOCKER_SECCOMP=default` (or a custom profile). Default networking uses the Docker bridge with internet egress; set `SANDBOX_DOCKER_NETWORK=astraforge-sandbox` for internal-only isolation or `SANDBOX_DOCKER_NETWORK=none` for no networking.
...
- Keep sandbox network isolated when required: use an internal bridge or NetworkPolicy that blocks RFC1918.
