...
- Docker sandboxes: read-only rootfs, tmpfs for `/workspace`/`/tmp`/`/run`, `--cap-drop=ALL`, `--security-opt=no-new-privileges:true`, seccomp profile (`SANDBOX_DOCKER_SECCOMP`), `--pids-limit`, and three network modes (default bridge egress; `SANDBOX_DOCKER_NETWORK=astraforge-sandbox` for internal-only; `SANDBOX_DOCKER_NETWORK=none` for no networking).
