...
- **Sandbox orchestrator**: provisions Docker or Kubernetes sandboxes with read-only roots, tmpfs workspaces, dropped capabilities, and network policies (Docker defaults to bridge egress unless `SANDBOX_DOCKER_NETWORK` is set to an internal bridge). Handles shell, upload, snapshot, and heartbeat APIs.
